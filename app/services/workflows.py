...
def _spreadsheet_workflow_context(
    analysis: SpreadsheetSourceResponse,
) -> dict[str, Any]:
    return {
        "source_spreadsheet_path": analysis.source_spreadsheet_path,
        "file_name": analysis.file_name,
        "analysis": analysis,
    }


def _run_spreadsheet_workflow_step(step: dict[str, Any], context: dict[str, Any]) -> None:
    tool_name = str(step.get("tool") or "").strip()
    bind_name = str(step.get("bind") or "").strip()
    needs = [str(item).strip() for item in step.get("needs", []) if str(item).strip()]
    ...
    for need in needs:
        value = context.get(need)
        if value in (None, "", []):
            raise RuntimeError(f"Spreadsheet workflow step `{tool_name}` is missing required context `{need}`.")
    binding = _workflow_binding_for_tool(tool_name, source_type="spreadsheet")
    if binding is None:
        raise NotImplementedError(f"Unsupported spreadsheet workflow step tool: {tool_name}")
    payload = _build_tool_payload_from_binding(binding, context)
    result = run_tool(tool_name, payload)
    value = _extract_tool_result_value(result, binding)
    transformed_value = transform_bound_value(str(binding.get("transform") or "identity"), value)
    ...
    used_tools_label = str(binding.get("used_tools_label") or tool_name).strip()
    analysis_obj = context.get("analysis")
    if used_tools_label and isinstance(analysis_obj, SpreadsheetSourceResponse):
        analysis_obj.used_tools.append(used_tools_label)


def _run_registered_spreadsheet_workflow_from_manifest(
    analysis: SpreadsheetSourceResponse,
    manifest: dict[str, object],
) -> dict[str, object]:
    context = _spreadsheet_workflow_context(analysis)
    steps = manifest.get("steps")
    if not isinstance(steps, list):
        raise ValueError(f"Workflow {manifest.get('name')} does not define a valid step list.")
    for step in steps:
        if not isinstance(step, dict):
            raise ValueError(f"Workflow {manifest.get('name')} contains a non-object step.")
        _run_spreadsheet_workflow_step(step, context)
...
def run_registered_spreadsheet_workflow(
    workflow_name: str,
    analysis: SpreadsheetSourceResponse,
) -> dict[str, object]:
    manifest = load_workflow_manifest(workflow_name)
    if manifest is None:
        raise ValueError(f"Unknown spreadsheet workflow: {workflow_name}")
    source_type = str(manifest.get("source_type") or "").strip().lower()
    if source_type != "spreadsheet":
        raise ValueError(f"Workflow {workflow_name} is not registered for spreadsheet sources.")

    requires = [str(item).strip() for item in manifest.get("requires", []) if str(item).strip()]
    if "source_spreadsheet_path" in requires and not analysis.source_spreadsheet_path:
        raise RuntimeError(
            "The active spreadsheet session does not expose a durable source file path, so this workflow cannot be rerun from chat."
        )
